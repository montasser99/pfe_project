...
@section('content')
    ...
    @endif--}}
                        ...
                        <tr>
                            @if (!$Exceptionnel->isEmpty())
                                <th style="text-align: center; color:rgb(16, 134, 51);">Exceptionnel :
                                    {{ $Exceptionnel[0]->CCONG_SOLDE_9 ?? null }} jours</th>
                            @else
                            <th> No solde ajouté  </th>

                            @endif
                            @if (!$Recuperation->isEmpty())
                                <th style="text-align: center; color:rgb(16, 134, 51);">Recuperation :
                                    {{ $Recuperation[0]->CCONG_SOLDE_9 ?? null }} jours</th>
                                    @else
                                    <th> No solde ajouté  </th>
                            @endif
                            @if (!$Annuel->isEmpty())
                                <th style="text-align: center; color:rgb(16, 134, 51);">Annuel :
                                    {{ $Annuel[0]->CCONG_SOLDE_9 ?? null }} jours</th>
                                    @else
                                    <th> No solde ajouté  </th>
                            @endif
                        </tr>
                ...
                margin-left: 5px;
                padding: 5.4px 14px;
                     font-size: 14px;     border-radius: 70px;"
                     @if (Auth::user()->role=="user") href="{{ route('Demandeconges.index') }}"
                     @else href="{{ route('DemandeAdmin.index') }}" @endif>
                     retour </a>
                    </div>
                </div>
            </div> <!-- END: Panel Body -->
